...
module.exports.clientController = {
    ...
    //ДОБАВИТЬ В КОРЗИНУ КЛИЕНТА

    takeBook: async (req, res) => {
        try {
            const client = await Client.findById(req.params.id);
            const book = await Book.findById(req.body.book)            // utochnit pochemu req body a ne paramss
            if (client.isBlocked) {
            ...
            if (book.rented) {                                           // renred bolean chto li?
    ...
    // УДАЛЕНИЕ КНИГИ ИЗ КОРЗИНЫ КЛИНТА
